    ...
    def evolve(self, epoch):
        ...
        for idx in range(0, self.pop_size):
            ...
            if self.mode not in self.AVAILABLE_MODES:
                ...
                pop_new[-1] = self.get_better_solution([pos_new, target, np.zeros(self.problem.n_dims)], self.pop[idx])
        ...
        for idx in range(0, self.pop_size):
            ...
            if self.compare_agent(pop_new[idr], pop_new[idx]) and np.random.rand() < self.p_hi:
                temp = self.global_move[0] * pop_new[idx][self.ID_POS] + self.global_move[1] * pop_new[idr][self.ID_POS]
                pos_new = self.amend_position(temp, self.problem.lb, self.problem.ub)
                target = self.get_target_wrapper(pos_new)
                nfe_epoch += 1
                if self.compare_agent([pos_new, target], pop_new[idx]):
                    pop_new[idx] = [pos_new, target]
        ...
        pop_child = []
        for idx in range(0, self.pop_size):
            dist_to_worst = np.linalg.norm(pop_new[idx][self.ID_POS] - g_worst[self.ID_POS])
            dist_to_best = np.linalg.norm(pop_new[idx][self.ID_POS] - g_best[self.ID_POS])

            ### 3. Starvation avoidance
            if dist_to_worst < self.delta[0]:
                temp = pop_new[idx][self.ID_POS] + np.random.uniform() * (self.problem.ub - self.problem.lb) * \
