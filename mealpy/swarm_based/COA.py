...
class OriginalCOA(Optimizer):
    ...
    n_coyotes : int
        Good range [3, 15], number of coyotes per group, default=5

    References
    ~~~~~~~~~~
    1. Pierezan, J. and Coelho, L.D.S., 2018, July. Coyote optimization algorithm: a new metaheuristic for
       global optimization problems. In 2018 IEEE congress on evolutionary computation (CEC) (pp. 1-8). IEEE.
    ...
    OPT_INFO = OptInfo(name="Coyote Optimization Algorithm", year=2018, difficulty="medium", kind="original")
    ...
    def evolve(self, epoch):
        ...
        for p in range(self.n_packs):
            ...
            self.pop_group[p], _ = self.get_sorted_population(self.pop_group[p], self.problem.minmax)
            ...
            packs, _ = self.get_sorted_population(self.pop_group[p], self.problem.minmax)
