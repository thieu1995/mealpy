...
class OriginalFA(Optimizer):
    ...
    max_sparks : int
        Parameter controlling the total number of sparks generated by the pop_size fireworks, in range [2, 10000]. Default is 100.
    p_a : float
        Percent (const parameter), in range (0.0, 1.0). Default is 0.04.
    p_b : float
        Percent (const parameter), in range (0.0, 1.0). Default is 0.8.
    max_ea : int
        Maximum explosion amplitude, in range [2, 100]. Default is 40.
    m_sparks : int
        Number of sparks generated in each explosion generation, in range [2, 10000]. Default is 100.

    References
    ~~~~~~~~~~
    1. Tan, Y. and Zhu, Y., 2010, June. Fireworks algorithm for optimization. In International conference in
       swarm intelligence (pp. 355-364). Springer, Berlin, Heidelberg. https://doi.org/10.1007/978-3-642-13495-1_44
